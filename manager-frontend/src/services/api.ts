...
export class ApiService {
    ...
    checkLogin() {
      return this.handlePost('checkLogin', undefined, { responseType: 'text' });
    }
    getAllNode() {
    ...
    handleGet(url: string, opts?: any) {
      ...
      return axios({method: 'get', url: 'http://localhost:8000'+url}).then(result => {
            console.log(url + " GET ")
            return result
        })
      // return this.http.get(url, opts).catch(err => this.handleError(err));
    }
    ...
    handleReq(addr: string, api: string, data?: FormData, opts?: any) {
      ...
      addr = 'http://' + addr + api;
      return this.handlePost(`${this.reqUrl}?addr=${addr}`, data, {port: 8080});
    }
